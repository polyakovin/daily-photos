...
В режиме «Карта» приложение читает GPS из EXIF и позволяет поставить или исправить точку прямо на общей карте либо в просмотрщике фотографии. Ручные и импортированные координаты хранятся в `photo_locations.json` в корне выбранной папки. Фотографии записываются туда по относительным путям, поэтому метки переносятся вместе с архивом; ручная точка имеет приоритет над EXIF, а EXIF — над координатами, автоматически сопоставленными из внешних источников. В режиме автоматического поиска по компьютеру файл остаётся в системном профиле приложения.
