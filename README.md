...
В macOS-настройках выбранной папки можно включить опцию «Конвертировать и заменять оригиналы». Тогда JPG, PNG, GIF, AVIF, HEIC и HEIF внутри этой папки рекурсивно конвертируются в WebP, проверяются и только после успешной проверки заменяют исходные файлы. Опция по умолчанию выключена, требует отдельного подтверждения и недоступна для автоматического поиска по нескольким папкам. Для конвертации в системе должны быть установлены `webp`, `ffmpeg`, `exiftool` и, для HEIC/HEIF, `libheif`.
