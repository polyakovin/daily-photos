...
test('десктопный сервер отдаёт вспомогательные UI-скрипты', async (t) => {
  ...
  assert.match(appScript, /RECENT_PLACE_STORAGE_KEY/);
  assert.match(appScript, /rememberRecentPlace/);
  ...
  assert.match(appScript, /event\.code === 'KeyM'/);
  assert.match(appScript, /startMapPhotoPlacement\(\)/);
  ...
  assert.match(appScript, /mapAssignmentSaving = false;\s*if \(completedSinglePlacement\)[\s\S]*?renderMapAssignment\(\)/);
  ...
  assert.match(
    appScript,
    /viewerDateInput\.addEventListener\('date-picker-select',[\s\S]*?changeViewerPhotoDate\(\)/
  );
