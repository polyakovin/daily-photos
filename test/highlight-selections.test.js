...
test('отметки периода и презентации хранятся отдельно в каждой папке архива', async (t) => {
  ...
  assert.equal(monthState.selections.months['2024-07'], julyPhoto.date);
  ...
  assert.deepEqual(JSON.parse(fs.readFileSync(archiveSelectionsFile, 'utf8')), {
    years: { '2024': augustPhoto.date },
    months: { '2024-07': julyPhoto.date }
  });
  ...
  assert.equal(restored.months['2024-07'], julyPhoto.date);
  assert.equal(restored.years['2024'], augustPhoto.date);
  ...
  assert.equal(returnedSelections.months['2024-07'], julyPhoto.date);
  assert.equal(returnedSelections.years['2024'], augustPhoto.date);
  assert.deepEqual(await fetch(`${server.url}/api/blur-dates`).then((response) => response.json()), ['2024-07-05']);
...
test('старые ID отметок периода автоматически заменяются на даты', async (t) => {
  const temporaryRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'photo-day-highlight-migration-'));
  const archiveRoot = path.join(temporaryRoot, 'archive');
  const stateRoot = path.join(temporaryRoot, 'state');
  const julyPath = path.join(archiveRoot, '2024-07-05 first.jpg');
  const augustPath = path.join(archiveRoot, '2024-08-09 second.jpg');
  fs.mkdirSync(archiveRoot, { recursive: true });
  fs.writeFileSync(julyPath, 'first-photo');
  fs.writeFileSync(augustPath, 'second-photo');
  const photoId = (filePath) => require('node:crypto')
    .createHash('sha256')
    .update(path.resolve(filePath))
    .digest('hex')
    .slice(0, 32);
  fs.writeFileSync(path.join(archiveRoot, 'period_photo_selections.json'), JSON.stringify({
    years: { '2024': photoId(augustPath) },
    months: { '2024-07': photoId(julyPath) }
  }));

  const server = await startPhotoDayServer({
    contentRoot: archiveRoot,
    stateRoot,
    convertImages: false,
    metadataIndex: true,
  ...
  const expected = {
    years: { '2024': '2024-08-09' },
    months: { '2024-07': '2024-07-05' }
  };
  assert.deepEqual(
    JSON.parse(fs.readFileSync(path.join(archiveRoot, 'period_photo_selections.json'), 'utf8')),
    expected
  );
  assert.deepEqual(
    await fetch(`${server.url}/api/highlight-selections`).then((response) => response.json()),
    expected
  );
});
